    ...
    for i in range(num_routes):
        try:
            if i == 0:
                ...
                # Alternative routes by temporarily removing some edges/nodes from the shortest path
                temp_G = G.copy()
                for prev_route, _ in routes:
                    # Remove some random edges/nodes from previous routes
                    edges_to_remove = [(prev_route[j], prev_route[j+1]) for j in range(len(prev_route)-1)]
                    temp_G.remove_edges_from(edges_to_remove)
                route = nx.shortest_path(temp_G, start_node, end_node, weight='length')
                distance = nx.shortest_path_length(temp_G, start_node, end_node, weight='length')
